...
def register_student_api(request):
	"""API endpoint for student registration from mobile app."""
	if request.method != 'POST':
		return JsonResponse({'error': 'Only POST allowed'}, status=405)
	...
	try:
		...
		if not all([first_name, last_name, email, password, roll_number, department, year_of_study]):
		...
		if UserProfile.objects.filter(student_id__iexact=roll_number).exists():
...
def upload_face_images(request):
	...
	# Create dataset directory using roll_number as student_id
...
# Attendance System
from .models import Attendance, UnknownPerson
